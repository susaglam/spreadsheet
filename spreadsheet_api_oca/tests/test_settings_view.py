...
class TestApiSettingsView(TransactionCase):
    """REST API settings live in the app owned by spreadsheet_oca.

    The module used to depend on spreadsheet_kpi_alert_oca only to reach the
    settings app that module declared; installing the API forced KPI alerts
    (models, cron, menus) onto every database.
    """

    def test_no_kpi_alert_dependency(self):
        self.assertNotIn(
            "spreadsheet_kpi_alert_oca",
            get_manifest("spreadsheet_api_oca")["depends"],
        )

    def test_block_inside_single_spreadsheet_app(self):
        view = self.env.ref(
            "spreadsheet_api_oca.res_config_settings_view_form_spreadsheet_api"
        )
        self.assertEqual(
            view.inherit_id,
        ...
        block = apps[0].xpath("./block[@name='api_settings_block']")
        self.assertEqual(len(block), 1)
        for field_name in (
            "api_default_rate_limit",
            "api_webhook_retry_max",
