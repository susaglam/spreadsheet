...
class TestRefreshSettingsView(TransactionCase):
    """Scheduled Refresh settings live in the app owned by spreadsheet_oca."""

    def test_block_inside_single_spreadsheet_app(self):
        view = self.env.ref(
            "spreadsheet_scheduled_refresh_oca.res_config_settings_view_form_spreadsheet_refresh"
        )
        self.assertEqual(
            view.inherit_id,
        ...
        block = apps[0].xpath("./block[@name='refresh_settings_block']")
        self.assertEqual(len(block), 1)
        self.assertTrue(
            block[0].xpath(".//field[@name='refresh_default_interval_hours']")
        )
